...
smd(
  ...
  async (message) => {
    try {
      let menuMessage = ` 
➮ᴇꜱ-ᴛᴇᴀᴍꜱ ʀᴜɴᴛɪᴍᴇ - ${runtime(process.uptime())} 
➮ᴇꜱ-ᴛᴇᴀᴍꜱ ᴅᴀᴛᴇ - ${message.date} 
➮ᴇꜱ-ᴛᴇᴀᴍꜱ ɴᴏᴡ ᴛɪᴍᴇ - ${message.time} 
➮Fᴏᴜɴᴅᴇʀ- *ᴇꜱ ᴛᴇᴀᴍꜱ-ᴛᴇᴄʜ👑*
➮Oᴡɴᴇʀ - ${Config.ownername} 
➮Nᴜᴍ - ${owner.split(",")[0]} 
➮Mᴇᴍᴏ - ${formatp(os.totalmem() - os.freemem())} 
      \n *𝔼𝕊 𝕋𝔼𝔸𝕄𝕊-𝕍2👑 SIMPLE WHATSAPP BOT*\n\n ${readmore} 
╭──❰ *ES TEAMS ALL MENU* ❱ 
│🎗 Lɪꜱᴛ 
│🎗 Cᴀᴛᴇɢᴏʀʏ 
│🎗 Hᴇʟᴘ 
...
astro_patch.cmd(
  ...
  async (message) => {
    try {
      ...
      let listMessage = `\n  
╭━━〘 * ${Config.botname} * 〙    
┃ 🎗 ES TEAM PREFIX: ${Config.HANDLERS}
┃ 🎗 ES TEAM OWNER: ${Config.ownername}
┃ 🎗 ES TEAM COMMAND: ${commands.length}
┃ 🎗 ES TEAM UPTIME: ${runtime(process.uptime())}
