  ...
  async (message, args) => {
    try {
      ...
      if (url.toLowerCase().includes("https")) {
        ...
        return await message.send("`𝔼𝕊 𝕋𝔼𝔸𝕄𝕊-𝕍2👑- UPDATE WAS SUCCESSFULL ✅`");
      } else {
...
${menuThemeFooter}                         
*┌┤✑  Thanks for Choosing ES TEAMS_V2👑
│└────────────┈ ⳹        
│*©2024-2099 ES TEAMS*
└─────────────────┈ ⳹*
${readmore}`;
...
astro_patch.smd(
  ...
  async (context) => {
    ...
    await context.send(
      `*𝔼𝕊 𝕋𝔼𝔸𝕄𝕊-𝕍2👑 ping is: ${pingTime} ᴍs*`,
      { edit: messageKey },
      "",
      context
    );
    ...
    try {
      ...
      const designs = [
        async () => {
          ...
          const captionText = `𝔼𝕊 𝕋𝔼𝔸𝕄𝕊-𝕍2👑 1.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds${quoteText}\n\n𝔼𝕊 𝕋𝔼𝔸𝕄𝕊-𝕍2👑`;
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*𝔼𝕊 𝕋𝔼𝔸𝕄𝕊-𝕍2👑 ʀᴜɴɴɪɴɢ sɪɴᴄᴇ:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "𝔼𝕊 𝕋𝔼𝔸𝕄𝕊-𝕍2👑 Running Since",
        body: message,
        footerText: "𝔼𝕊 𝕋𝔼𝔸𝕄𝕊-𝕍2👑 2024",
